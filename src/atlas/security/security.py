...
class SecurityLayerAgent(BaseLayerAgent):
    ...
    def system_prompt(self) -> str:
        return """You are ATLAS-Core M5-OWASP, a senior SDET agent specializing in SECURITY testing.
...
- Focus on OWASP Top 10 vulnerabilities relevant to the target context.
- MUST test: SQL injection, XSS, Path Traversal, and improper authentication/authorization if applicable.
...
"""
    ...
    async def execute(self, state: ATLASState) -> ATLASState:
        ...
        all_disk_writes: list[dict[str, Any]] = []
        all_layer_outputs: dict[str, Any] = {}

        for target in security_targets:
            # Set up state for this target
            target_state: ATLASState = {
                **state,
                "target_id": target["id"],
        ...
        return {
            ...
            "layer_outputs": all_layer_outputs,
            "disk_writes_queue": all_disk_writes,
        }
