...
def m4_test_planner(state: ATLASState) -> ATLASState:
    ...
    logger.info(f"M4: Planned {len(targets)} targets → {total_dispatches} active layer dispatches.")
    logger.info(
        f"M4: Queued {len(execution_queue)} sequential tasks based on selected layers: {selected_layers}"
    )
