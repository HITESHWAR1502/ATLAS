...
async def m1_ingestion(state: ATCGState, config: ATCGConfig, db: NeonConnection) -> ATCGState:
    """
    M1: Ingestion + Neon Fixture Registry Query.

    Detects project context, queries Neon for fixtures and history.

    Updates state with:
        - project_context: Language, framework, conventions
        - neon_fixtures: Pre-fetched shared fixtures from registry
        - neon_history: Prior test runs for changed targets
    """
    project_root = Path(
    ...
    for file_path in changed_files[:10]:  # Sample first 10 files
        full_path = project_root / file_path
        if full_path.exists():
            try:
                content = full_path.read_text(encoding="utf-8", errors="replace")
                # Quick heuristic for dependency tags
                if "neon" in content.lower() or "postgres" in content.lower():
                    dependency_tags.extend(["neon", "db", "postgres"])
                if "redis" in content.lower():
                    dependency_tags.append("redis")
                if "auth" in content.lower() or "jwt" in content.lower():
                    dependency_tags.extend(["auth", "jwt"])
                if "http" in content.lower() or "fetch" in content.lower():
                    dependency_tags.append("http")
            except Exception:
                pass

    dependency_tags = list(set(dependency_tags))

    neon_fixtures: list[dict[str, Any]] = []
    try:
        neon_fixtures = await query_fixtures(
            db=db,
            project_name=project_name,
            language=language,
            framework=test_framework,
            dependency_tags=dependency_tags if dependency_tags else None,
        )
    except Exception as e:
        logger.warning(f"Failed to query fixture registry: {e}")

    # ── Query Neon test history ───────────────────────────────────────────────
    neon_history: list[dict[str, Any]] = []
    try:
        import asyncio
        # Fetch history for all changed targets in parallel
        tasks = []
        for file_path in changed_files[:20]:  # Limit to 20 files
            target_id = file_path.replace("/", ".").replace("\\", ".").rsplit(".", 1)[0]
            tasks.append(get_target_history(db, target_id))
        
        histories = await asyncio.gather(*tasks)
        for history in histories:
            neon_history.extend(history)
    except Exception as e:
        logger.warning(f"Failed to query test history: {e}")

    logger.info(
        f"M1: Loaded {len(neon_fixtures)} fixtures, "
        f"{len(neon_history)} history records from Neon"
    )

    return {
        **state,
        "project_context": project_context,
        "neon_fixtures": neon_fixtures,
        "neon_history": neon_history,
    }
