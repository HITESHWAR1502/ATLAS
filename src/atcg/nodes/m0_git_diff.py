...
This is the entry point of the ATCG pipeline. It analyzes the git repository
...
def m0_git_diff_filter(state: ATCGState) -> ATCGState:
    """
    ...
    try:
        ...
        for diff_item in unstaged_diffs:
            file_path = diff_item.b_path or diff_item.a_path
            if file_path and _is_source_file(file_path) and file_path not in changed_files:
                changed_files.append(file_path)
                diff_hunks.append({
                    "file": file_path,
                    "change_type": diff_item.change_type or "M",
