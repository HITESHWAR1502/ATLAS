...
from atcg.state import ATCGState, FunctionClassification
...
DB_PATTERNS = [
    "query", "execute", "fetch", "fetchone", "fetchall", "fetchrow", "fetchval",
    "insert", "update", "delete", "select", "commit", "rollback", "transaction",
    "cursor", "connection", "pool", "neon", "postgres", "mysql", "sqlite",
    "prisma", "drizzle", "sequelize", "sqlalchemy", "typeorm",
]

AUTH_PATTERNS = [
    "auth", "login", "logout", "signin", "signup", "register",
    "token", "jwt", "session", "cookie", "password", "credential",
    "bcrypt", "hash_password", "verify_password", "check_password",
    "permission", "role", "acl", "rbac", "scope",
]

HTTP_PATTERNS = [
    "fetch", "request", "axios", "http", "https",
...
CACHE_PATTERNS = [
    "cache", "redis", "memcached", "get_cache", "set_cache",
    "invalidate", "ttl", "expire",
]

FILE_PATTERNS = [
    "open(", "read_file", "write_file", "os.path",
    "fs.readFile", "fs.writeFile", "path.join",
    "upload", "download", "file_path",
]
...
def _classify_function(
    ...
    route_decorators = ["@app.", "@router.", "@get", "@post", "@put", "@delete",
...
def m2_ast_parser(state: ATCGState) -> ATCGState:
    """
    M2: AST Parser node.
    ...
    for file_path in changed_files:
        ...
        has_neon = any(
            kw in source.lower()
            for kw in ["neon", "@neondatabase", "asyncpg", "psycopg"]
        )

        module_contexts.append({
            "module_path": file_path,
            "imports": imports,
            "dependencies": list({dep for fn in functions for dep in fn.get("dependencies", [])}),
            "has_neon_imports": has_neon,
            "functions_count": len(functions),
            "vulnerable_imports": vulnerable_imports,
        })

    logger.info(
        f"M2: Parsed {len(changed_files)} files → "
        f"{len(all_functions)} functions extracted"
    )
